		...
		<div className="flex gap-0.5">
			{[...Array(5)].map((_, index) => {
				...
				if (rating >= starRating) {
					...
					return (
						<Star key={starNumber} className={`${iconClass} text-gray-300`} />
					);
				}
			})}
		</div>
...
function buildCriteriaList(
	...
	const preferred: Array<keyof ReviewItemProps["criteries"]> = [
		"language",
		"story",
		"theme",
		"characters",
...
export function ReviewItem({
	...
	return (
		<div
			className={`w-full min-w-0 px-3 sm:px-4 py-3 rounded-xl sm:rounded-2xl flex flex-col gap-3 sm:gap-4`}
		>
			...
			<div className="flex items-center justify-between gap-2 pt-2 sm:pt-4 border-t border-border/30">
				...
				<div className="flex items-center gap-3">
					<div className="flex gap-1.5 items-center cursor-pointer hover:text-red-500 transition-colors">
						...
						<p className="text-muted-foreground min-w-6 text-center text-sm">
							{likes}
						</p>
					</div>
				</div>
