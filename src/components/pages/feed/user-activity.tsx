...
import { useToggleActivityReaction, useUserActivities } from "@/hooks/activity";
...
export function UserActivity({ userId }: UserActivityProps) {
  ...
  const toggleReaction = useToggleActivityReaction(userId);
    ...
    return (
      <Empty className="border-0">
        <EmptyHeader>
          ...
          <EmptyTitle>{t("user:noActivity")}</EmptyTitle>
          <EmptyDescription>{t("user:noActivityDescription")}</EmptyDescription>
        </EmptyHeader>
      </Empty>
    );
  }
