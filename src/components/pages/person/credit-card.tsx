...
const MEDIA_ICONS: Record<PersonCredit["mediaType"], string> = {
...
export function CreditCard({ credit }: { credit: PersonCredit }) {
  ...
  const roles = credit.roles.filter(Boolean).join(", ");
      ...
      <div className="relative aspect-3/4 overflow-hidden rounded-lg border border-border bg-muted">
        <div
          className={cn(
            ...
            credit.isAdult && "blur-md group-hover:blur-none",
          )}
          style={{ backgroundImage: `url("${credit.imageUrl || "/placeholder/cover.webp"}")` }}
        />
        ...
        {credit.episodeCount !== null && credit.episodeCount > 1 && (
          <span className="absolute right-1.5 bottom-1.5 rounded-md bg-black/65 px-1.5 py-0.5 text-white text-xs backdrop-blur-sm">
            {t("library:episodeCount", { count: credit.episodeCount })}
          </span>
        )}
      </div>
