...
type ProgressStatus = "Planning" | "Reading" | "Rereading" | "Completed" | "Paused" | "Dropped";
...
export function MangaModal({ mangaId, totalChapters, onClose }: MangaModalProps) {
  ...
  const progressQuery = useQuery<MangaProgressData | null>({
    queryKey: ["mangaProgress", mangaId, userId],
    queryFn: () =>
      api
        .get(apiEndpoints.getMangaProgress(userId as string, mangaId as string))
        .then(({ data }) => data.mangaProgresses.items[0] ?? null),
    enabled,
  });

  const reviewQuery = useQuery<ApiTypes.Review | null>({
    queryKey: ["mangaReview", mangaId, userId],
    queryFn: () =>
      api
        .get<ApiTypes.GetReviewsResponse>(`${apiEndpoints.mangaReview}/?mangaId=${mangaId}&userId=${userId}`)
        .then(({ data }) => data.mangaReviews?.items[0] ?? null),
    enabled,
  });
  ...
  const listStatusQuery = useQuery<string[]>({
    queryKey: ["mangaListStatus", mangaId, userId],
    queryFn: () =>
      api
        .get<ApiTypes.GetListStatusResponse>(apiEndpoints.getListStatus, {
          params: { type: "Manga", mangaId },
        })
  ...
  useEffect(() => {
    ...
    progressForm.reset({
      ...
      chaptersRead: progress.chaptersRead != null ? String(progress.chaptersRead) : "",
  ...
  useEffect(() => {
    ...
    reviewForm.reset({
      ...
      art: review.art != null ? String(Number(review.art)) : "0",
      worldbuilding: review.worldbuilding != null ? String(Number(review.worldbuilding)) : "0",
  ...
  const saveProgressMutation = useMutation({
    mutationFn: (data: ProgressFormData) => {
      ...
      return api.post(apiEndpoints.mangaProgress, {
        mangaId,
        status,
        chaptersRead: data.chaptersRead.trim() === "" ? undefined : Number(data.chaptersRead),
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["mangaProgress", mangaId, userId] });
  ...
  const saveReviewMutation = useMutation({
    mutationFn: (data: ReviewFormData) => {
      const body = {
        ...
        art: Number(data.art) || undefined,
        worldbuilding: Number(data.worldbuilding) || undefined,
      ...
      return existing
        ? api.patch(`${apiEndpoints.mangaReview}/${existing.id}`, body)
        : api.post(apiEndpoints.mangaReview, body);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["mangaReview", mangaId, userId] });
      queryClient.invalidateQueries({ queryKey: ["mangaReviews", mangaId] });
      queryClient.invalidateQueries({ queryKey: ["manga"] });
    },
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      if (reviewQuery.data) {
        await api.delete(`${apiEndpoints.mangaReview}/${reviewQuery.data.id}`);
      }
      if (progressQuery.data) {
        await api.delete(`${apiEndpoints.mangaProgress}/${progressQuery.data.id}`);
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["mangaProgress", mangaId, userId] });
  ...
  const toggleListMutation = useMutation({
    mutationFn: ({ listId, isMember }: { listId: string; isMember: boolean }) => {
      const body = { type: "Manga", listId, userId, mangaId };
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["mangaListStatus", mangaId, userId] });
      queryClient.invalidateQueries({ queryKey: ["mangaContainingLists", mangaId] });
    },
    onError: () => toast.error(t("api:INTERNAL_SERVER_ERROR")),
  });

  const createListMutation = useMutation({
    mutationFn: (name: string) => api.post(apiEndpoints.list, { name, userId, type: "Manga" }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["mangaLists", userId] }),
    onError: () => toast.error(t("api:INTERNAL_SERVER_ERROR")),
  });
  ...
  const handleStatusChange = (onChange: (value: string) => void) => (value: string) => {
    ...
    if (value === "completed" && totalChapters) {
      progressForm.setValue("chaptersRead", String(totalChapters));
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-muted/30 rounded-lg p-4 border border-border/50">
          ...
          <div className="space-y-3">
            ...
            <Field>
              <FieldLabel htmlFor="chapters" className="text-sm font-medium">
                {t("library:chapters")}
              </FieldLabel>
              <InputGroup className="bg-background">
                ...
                {!!totalChapters && (
                  <InputGroupAddon align="inline-end">
                    <InputGroupText>/{totalChapters}</InputGroupText>
                  </InputGroupAddon>
                )}
              </InputGroup>
            </Field>
        ...
        <div className="bg-muted/30 rounded-lg p-4 border border-border/50">
          ...
          <div className="flex flex-col gap-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">{t("feed:criteries.art")}</span>
                <Controller
                  control={reviewForm.control}
                  name="art"
                  render={({ field }) => (
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">{t("feed:criteries.worldbuilding")}</span>
                <Controller
                  control={reviewForm.control}
                  name="worldbuilding"
                  render={({ field }) => (
