...
function LinkTabs({ className, children, ...props }: LinkTabsProps) {
  return (
    <div data-slot="link-tabs" className={cn("flex flex-col gap-2", className)} {...props}>
      {children}
    </div>
  );
}
...
function LinkTabsTrigger({
  ...
  const isActive = activeMatchFn ? activeMatchFn(location.pathname, to) : location.pathname === to;
