...
export const Route = createFileRoute("/account-deleted")({
  head: () => ({
    meta: [...seo({ title: "Account Deleted" })],
  }),
      ...
      <Empty>
        <EmptyHeader>
          <EmptyTitle className="mask-b-from-20% mask-b-to-80% font-extrabold text-9xl text-primary">
            <Icon icon={"lucide:trash-2"} className="size-24" />
          </EmptyTitle>
          <EmptyTitle className="text-2xl font-bold text-white">
            <Trans i18nKey={"pages:accountDeleted.title"} />
          </EmptyTitle>
          <EmptyDescription className="text-white">
            <Trans i18nKey={"pages:accountDeleted.description"} />
          </EmptyDescription>
        </EmptyHeader>
        <EmptyContent>
