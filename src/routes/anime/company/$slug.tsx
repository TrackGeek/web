...
export const Route = createFileRoute("/anime/company/$slug")({
  loader: async ({ params }) => {
    const company: Company = await api.get(apiEndpoints.getAnimeCompany(params.slug)).then(({ data }) => data.company);
    return { company };
  },
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        ...seo({
          title: company?.name ? `${company.name} Anime` : "Anime Studio",
          description:
  ...
  component: AnimeCompanyRoute,
  errorComponent: ErrorComponent,
  notFoundComponent: NotFoundComponent,
});

function AnimeCompanyRoute() {
