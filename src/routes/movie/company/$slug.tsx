...
export const Route = createFileRoute("/movie/company/$slug")({
  loader: async ({ params }) => {
    const company: Company = await api.get(apiEndpoints.getMovieCompany(params.slug)).then(({ data }) => data.company);
    return { company };
  },
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        ...seo({
          title: company?.name ? `${company.name} Movies` : "Movie Company",
          description:
  ...
  component: MovieCompanyRoute,
  errorComponent: ErrorComponent,
  notFoundComponent: NotFoundComponent,
});

function MovieCompanyRoute() {
