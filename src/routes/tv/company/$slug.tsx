...
export const Route = createFileRoute("/tv/company/$slug")({
  loader: async ({ params }) => {
    const company: Company = await api.get(apiEndpoints.getTvShowCompany(params.slug)).then(({ data }) => data.company);
    return { company };
  },
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        ...seo({
          title: company?.name ? `${company.name} TV Shows` : "TV Company",
          description:
  ...
  component: TVShowCompanyRoute,
  errorComponent: ErrorComponent,
  notFoundComponent: NotFoundComponent,
});

function TVShowCompanyRoute() {
